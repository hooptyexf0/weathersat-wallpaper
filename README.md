...
Includes previously added check for initial run, made for half hour runs, keeps rolling 96 images for a 48hr history.
Made to be run via Cron job (*/30 * * * *), setup not baked into script yet.

Must be made executable via chmod +x or GUI (Properties->Permissions)
